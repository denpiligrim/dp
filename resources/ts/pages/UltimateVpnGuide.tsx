...
export default function UltimateVpnGuide() {
  ...
  const relaySetupCode = useRelay ?
    `# Выполните этот код на Relay сервере (${relayIp})\n# Настраиваем проброс портов с ${relayDomain} на ${vpnIp}\n\nsysctl -w net.ipv4.ip_forward=1\niptables -t nat -A PREROUTING -p tcp --dport 443 -j DNAT --to-destination ${vpnIp}:443\niptables -t nat -A PREROUTING -p udp --dport 443 -j DNAT --to-destination ${vpnIp}:443\niptables -t nat -A POSTROUTING -j MASQUERADE` : '';
    ...
    <>
      ...
      <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: '1000px', mx: 'auto' }}>
        ...
        <Box component="article">
          ...
          <CodeBlock
            code={`ssh-keygen -t ed25519 -C "username@vps" -f "$env:USERPROFILE\\.ssh\\id_ed25519_vps"`}
            language="powershell"
          />
